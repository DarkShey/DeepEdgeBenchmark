...
accès réseau. Le seul point de contact avec la Partie A (Kyrio, validation_runner.py)
est save_prediction(record) et le contrat de champs de CONTRACT_FIELDS (§3 du brief).
...
def save_prediction(record: dict, db_path=DEFAULT_DB_PATH) -> bool:
    ...
    si doublon ignoré."""
    missing = [f for f in CONTRACT_FIELDS if f not in record]
    if missing:
    ...
    conn = _connect(db_path)
    try:
        placeholders = ", ".join(f":{f}" for f in CONTRACT_FIELDS)
        columns = ", ".join(CONTRACT_FIELDS)
        cur = conn.execute(
            f"INSERT OR IGNORE INTO predictions ({columns}) VALUES ({placeholders})",
            {f: record[f] for f in CONTRACT_FIELDS},
        )
        conn.commit()
