...
def generate_nsdiff_asset(asset: str, daily: pd.Series, weekly: pd.Series, weekly_dates: pd.Series,
    ...
    """`collect_samples=True` attaches each row's raw predictive cloud under the
    key `"samples"` (np.ndarray, length n_samples) IN ADDITION to the usual
    point/quantile fields -- opt-in, default off, so the oos/DB path below is
    byte-for-byte unchanged (`insert_oos_predictions` never sees the key).
    Needed by consumers that must recompute quantiles at other levels or warp
    the cloud (conformal recalibration, seed ensembling) rather than re-fit.
        ...
        for h in needed_h:
            ...
            if len(row_c):
                ...
                rows_c.append({
                    ...
                    **({"samples": np.asarray(s, dtype=float)} if collect_samples else {}),
                })
