"""
NsDiff-W — variante hebdomadaire du port NsDiff (mirror de tsdiff_weekly.py)
============================================================================
Resample daily -> weekly (W-FRI, dernière clôture), entraîne le denoiser
LSNM+UANS de NsDiff sur les rendements **hebdomadaires**, et génère
**W+1 / W+2 / W+3 en un seul tir** : le modèle produit tout le chemin
d'horizon d'un coup (non-autorégressif), donc AUCUNE récursion / AUCUN
compounding d'erreur -- exactement la même propriété que tsdiff_weekly.py,
pour la même raison (cf. son propre docstring).

Réutilise entièrement l'architecture de models/nsdiff_model.py (classe
NsDiff, LSNM, UANS...) -- on ne fait que changer la fréquence des données
(weekly) et lire l'horizon en semaines.

Additif : ne touche ni le pipeline ni le modèle daily.
"""

import numpy as np
import pandas as pd

import nsdiff_model as nm   # réutilise NsDiff, _log_returns, _make_windows, set_seed
...
def fit_weekly(train_weekly: pd.Series, seq_len=SEQ_LEN_W, horizon=HORIZON_W,
              hidden_mean=HIDDEN_MEAN_W, hidden_sigma=HIDDEN_SIGMA_W,
              hidden_denoise=HIDDEN_DENOISE_W, sigma_kernel=SIGMA_KERNEL_W,
              k_denoise=K_DENOISE_W, epochs=EPOCHS_W, batch_size=BATCH_W):
    """Entraîne un NsDiff sur les rendements hebdo standardisés. Retourne
    ...
    model = nm.NsDiff(seq_len=seq_len, horizon=horizon, hidden_mean=hidden_mean,
                      hidden_sigma=hidden_sigma, hidden_denoise=hidden_denoise,
                      sigma_kernel=sigma_kernel, T=k_denoise)
...
def forecast_from_fitted_weekly(model, mu: float, sd: float, lookback_z, last_price: float,
                                weeks=(1, 2, 3), n_samples=N_SAMPLES_W, **_ignored) -> dict:
    ...
    Retourne {k: {"point","lo","hi","samples"}}. `**_ignored` absorbe un éventuel
    kwarg `k_denoise` (le nombre de pas de diffusion de NsDiff est fixé au fit,
    cf. nsdiff_model.py) pour partager un appelant avec tsdiff_weekly.py."""
    paths = model.sample_paths(np.asarray(lookback_z, dtype=np.float32),
                               n_samples=n_samples)   # [N, horizon]
    out = {}
    for k in weeks:
...
def forecast_weekly(train_weekly: pd.Series, weeks=(1, 2, 3),
                    n_samples=N_SAMPLES_W, **fit_kw) -> dict:
    ...
    return forecast_from_fitted_weekly(model, mu, sd, z[-model.seq_len:], last,
                                       weeks=weeks, n_samples=n_samples)
